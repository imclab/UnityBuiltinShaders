...
inline half3 DecodeHDR_NoLinearSupportInSM2 (fixed4 data, half4 decodeInstructions)
{
	...
	#if (SHADER_TARGET < 30) || (defined(SHADER_API_GLES) && defined(SHADER_API_MOBILE))
...
*
*/
half4 BRDF1_Unity_PBS (half3 diffColor, half3 specColor, half oneMinusReflectivity, half oneMinusRoughness, half3 normal, half3 viewDir, UnityLight light, half3 specGI)
{
    ...
    half3 color =	diffColor * (light.ambient + light.color * diffuseTerm)
                    + specularTerm * light.color * FresnelTerm (specColor, vh)
					+ specGI * FresnelLerp (specColor, grazingTerm, nv);
...
half4 BRDF2_Unity_PBS (half3 diffColor, half3 specColor, half oneMinusReflectivity, half oneMinusRoughness, half3 normal, half3 viewDir, UnityLight light, half3 specGI)
{
	half roughness = 1-oneMinusRoughness;
	half3 halfDir = normalize (light.dir + viewDir);
...
half4 BRDF3_Unity_PBS (half3 diffColor, half3 specColor, half oneMinusReflectivity, half oneMinusRoughness, half3 normal, half3 viewDir, UnityLight light, half3 specGI)
{
	half roughness = 1-oneMinusRoughness;
	half3 halfDir = normalize (light.dir + viewDir);
